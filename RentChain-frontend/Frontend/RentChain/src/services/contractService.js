...
class ContractService {
  ...
  async addProperty(title, propertyType, rentAmount, ipfsHash, leaseAgreementIpfsHash) {
    ...
    try {
      const tx = await this.contracts.listings.addProperty(
        ...
        ethers.parseUnits(rentAmount.toString(), 6), // USDT has 6 decimals
        ipfsHash,
  ...
  async editProperty(propertyId, title, propertyType, rentAmount, ipfsHash, leaseAgreementIpfsHash) {
    ...
    try {
      const tx = await this.contracts.listings.editProperty(
        ...
        title,
        propertyType,
        ethers.parseUnits(rentAmount.toString(), 6),
        ipfsHash,
        leaseAgreementIpfsHash
      );
      const receipt = await tx.wait();
  ...
  async getProperty(propertyId) {
    ...
    try {
      ...
      return {
        ...
        rentAmount: ethers.formatUnits(property[4], 6),
  ...
  async getPayment(paymentId) {
    ...
    try {
      ...
      return {
        ...
        amount: ethers.formatUnits(payment[4], 6),
  ...
  async calculateFee(amount) {
    this.checkInitialized();
    try {
      const fee = await this.contracts.escrow.calculateFee(ethers.parseUnits(amount.toString(), 6));
